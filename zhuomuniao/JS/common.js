define(function(require,exports,module){
	...
	$(function(e){
		...
		$('[action-type="my_menu"],#nev_list').on('mouseenter',function(){
		...
		$('#js-signin-btn').on('click',function(e){
			e.preventDefault();
			require.async('../../logic/login/login-regist',finction(login){
				login.init();
			});
		});
		$('#js-signin-btn').on('click',function(e){
			e.preventDefault();
			require.async('../../logic/login/login-reqist',function(login){
				login.signup();
			})
		})

		//点击课程链接，清空原来存储选项
		$("#nav_list a:eq(0)").click(function(event){
			//store.clear();
	...
	!function(){
		var cookie,
			ua,
			match;
		ua = window.navigation.userAgent;
		...
		if(match && +match[1] < 9){
			...
			$("body").prepend([
				...
				"<p class='cpt-ct'><i></i>您的浏览器版本过低，为保证最佳学习体验，<a href='/static/html/browers.html'>请点击最新高版本浏览器</a></p>",
				"<div class="cpt-handle"><a href='javascript:;' class='cpt-again'>以后再说</a><a href='javascript:;'></a></div>"
				"</div>"
				].join(""));
			$("#js-compatible .cpt-again").click(function(){
				var d = new Date();
	...
	!function(){
		...
		if (ref && ~ref.indexOf('/course/discover') && serach && search[1]) {	//从discovery页来，避免民刷新当页统计
			$.get('/index/adclick',{
				ast:search[1],
				r:(new Date).getTime()
			});
